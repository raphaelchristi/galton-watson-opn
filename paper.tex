...
\title{The Cyclotomic Factor Cascade as a Supercritical Branching Process}
\author{Raphael Christi\\
Rio de Janeiro, Brazil\\
\texttt{raphaelvaldetaro@gmail.com}}
\date{}
...
\begin{abstract}
We model the factor chain induced by the third cyclotomic polynomial $\Phi_3(p) = p^2+p+1$
as a Galton--Watson branching process on the set of odd primes. For each prime~$p$ in a cascade
seeded by a finite set~$S_0$, we count the number of \emph{new} odd prime factors of $\Phi_3(p)$
not already present in the running set~$S$. Over $300$ seed configurations generating $12{,}927$
parent--child observations, we find a global mean offspring rate $\bar\mu = 1.1747$, placing the
process in the supercritical regime. We observe a phase transition: $\mu(p) < 1$ for primes
$p \lesssim 200$ and $\mu(p) > 1$ for $p \gtrsim 200$, consistent with Stewart's theorem that
$P^+(\Phi_3(p)) > p^{2-\varepsilon}$ for large~$p$. The offspring rate increases further for higher
cyclotomic polynomials ($\mu \approx 2.99$ for $\Phi_5$, $\mu \approx 1.71$ for $\Phi_7$). These
measurements provide a quantitative explanation for the empirical observation that no finite set
of odd primes is closed under the $\sigma$-induced factor map, offering probabilistic evidence
against the existence of odd perfect numbers.
\end{abstract}
...
For a prime $p$ encountered at round $n$ of the cascade, define its \emph{offspring} as the set of odd primes dividing $\Phi_3(p)$ that are not already in $S_n$ at the time $p$ is processed:
\[
...
Over 300 cascades, we recorded $12{,}927$ parent--child observations:
...
Since $\bar\mu > 1$, the cascade is supercritical in the Galton--Watson sense. No cascade closed.
...
Our contribution is not the cascade itself, which is well established, but the probabilistic framing: (1) computing the empirical offspring distribution and mean $\mu$ across $12{,}927$ observations; (2) identifying the subcritical-to-supercritical phase transition near $p \approx 200$; (3) connecting $\mu(p) > 1$ to Stewart's theorem as the theoretical explanation; (4) measuring $\mu$ for $\Phi_5$ and $\Phi_7$, showing universality of supercriticality.
...
\begin{enumerate}
\item \textbf{Rigorous supercriticality.} Can one prove that $\mu(p) > 1$ for all primes $p > p_0$, without relying on the empirical distribution? Stewart's theorem guarantees at least one large child, but bounding the \emph{expected number} of new primes requires controlling the smooth part of $\Phi_3(p)$ as well.

\item \textbf{Correlation structure.} The offspring counts are not independent across primes in the same cascade. Quantifying the correlation decay---or proving it is negligible---would make the branching process model rigorous.

\item \textbf{Higher exponents in combination.} In a realistic OPN, multiple exponents are active simultaneously. The effective branching rate is the sum over all active $\Phi_n$. A joint analysis incorporating the constraint $\sum e_i \geq C(k)$ from known bounds could yield stronger results.

\item \textbf{Connection to abc.} The conjecture abc implies effective versions of Stewart's theorem. Does abc give a computable $N_0$ that closes the gap between the supercritical regime ($p > 200$ empirically) and the theoretical threshold?
\end{enumerate}

\section*{Acknowledgements}

The author thanks the community of odd perfect number researchers whose publicly available work made this investigation possible. Computational verification scripts are available at \url{https://github.com/raphaelchristi/galton-watson-opn}.
